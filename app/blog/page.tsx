...
export default function BlogPage() {
  ...
  // Group by year
  const grouped = filtered.reduce<Record<string, BlogPost[]>>((acc, post) => {
          ...
          <div className="p-4 lg:p-6">
            ...
            <div className="flex flex-col gap-3 mb-8">
              ...
              <div className="relative">
                <button
                  onClick={() => { setCategoryOpen((o) => !o); setTagOpen(false); }}
                  className="flex items-center justify-between w-full lg:w-72 px-5 py-3 rounded-2xl text-sm font-medium"
                  style={{
                ...
                {categoryOpen && (
                  <div
                    className="absolute z-20 mt-2 w-full lg:w-72 rounded-2xl overflow-hidden shadow-lg"
                    style={{ backgroundColor: "var(--background)", border: "1px solid var(--border)" }}
                  >
                    <button
                      onClick={() => { setSelectedCategory("all"); setCategoryOpen(false); }}
                      className="w-full text-left px-5 py-3 text-sm hover:opacity-80 transition-opacity"
                      style={{ color: selectedCategory === "all" ? "var(--gold)" : "var(--foreground)" }}
                    >
                      All Categories
                    </button>
                    {allCategories.map((cat) => (
                      <button
                        ...
                        className="w-full text-left px-5 py-3 text-sm hover:opacity-80 transition-opacity"
                        style={{ color: selectedCategory === cat ? "var(--gold)" : "var(--foreground-muted)" }}
                      >
                        {cat}
                      </button>
                    ))}
              ...
              <div className="relative">
                <button
                  onClick={() => { setTagOpen((o) => !o); setCategoryOpen(false); }}
                  className="flex items-center justify-between w-full lg:w-72 px-5 py-3 rounded-2xl text-sm font-medium"
                  style={{
                    backgroundColor: "var(--background)",
                    border: "1px solid var(--border)",
                ...
                {tagOpen && (
                  ...
                  >
                    <button
                      onClick={() => { setSelectedTag("all"); setTagOpen(false); }}
                      className="w-full text-left px-5 py-3 text-sm hover:opacity-80 transition-opacity"
                      style={{ color: selectedTag === "all" ? "var(--gold)" : "var(--foreground)" }}
                    >
                      All Tags
                    </button>
                    {allTags.map((tag) => (
                      <button
                        ...
                        className="w-full text-left px-5 py-3 text-sm hover:opacity-80 transition-opacity"
                        style={{ color: selectedTag === tag ? "var(--gold)" : "var(--foreground-muted)" }}
                      >
                        {tag}
                      </button>
                    ))}
                    ...
                    {grouped[year].map((post, i) => (
                      <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: "none" }}>
                        ...
                        >
                          ...
                          {/* Cover image */}
                          {post.coverImage && (
                            <div className="relative h-16 w-16 lg:h-20 lg:w-20 flex-shrink-0 overflow-hidden rounded-xl shadow-sm">
                              <Image
                                src={post.coverImage}
                                alt={post.title}
                                fill
